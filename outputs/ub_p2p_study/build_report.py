...
traffic=[]
for p,k,name in [('single',1,'单源'),('multi',4,'4 热种子'),('multi',8,'8 热种子'),('p2p',1,'P2P')]:
...
for key,name in limit_names.items():
    ...
    limits.append([name,f(a['time_s']),f(b['time_s']),f(a['time_s']/b['time_s'])+'×',f(b['cross_rack_gb'])])
...
values={
 ...
 'SCALE_TABLE':table(['带宽 GB/s','客户端数','单源秒','4 种子秒','8 种子秒','P2P 秒','单源/P2P'],scale),
 'TRAFFIC_TABLE':table(['方案','种子发出 GB','Peer 发出 GB','最终客户端缓存 GB','全网瞬时载荷峰值 GB/s'],traffic),
 'LIMITS_TABLE':table(['约束','单源 W=100 秒','P2P 秒','单源/P2P','P2P 跨架 GB'],limits),
 'CHUNKS_TABLE':table(['分块 MB（十进制）','块数','P2P 秒'],chunks),
 'WINDOW_TABLE':table(['每发送端活跃块 W','P2P 秒'],window),
 'COUNTER_TABLE':table(['条件，带宽均 GB/s','单源秒','P2P 秒'],counter),
